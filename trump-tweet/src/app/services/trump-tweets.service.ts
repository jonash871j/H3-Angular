...
export enum TweetFilter {
  All = 0,
  AllInSeptemberAndMoreThan100000Likes = 1
};
...
export class TrumpTweetsService {
  ...
  getTweetsBasedOnFilter(filter : TweetFilter) : Tweet[]{
    switch(+filter){
      case TweetFilter.All:
        return this.tweets;
      case TweetFilter.AllInSeptemberAndMoreThan100000Likes:
        return this.getTweetsAllInSeptemberAndMoreThan100000Likes();
      default:
        return [];
    }
  }

  getTweetsAllInSeptemberAndMoreThan100000Likes() : Tweet[]{
    let tweetsFilter : Tweet[] = [];
    for (let i = 0; i < this.tweets.length; i++){
      if (this.tweets[i].likes > 100000 && this.tweets[i].date.getMonth() == 8){
